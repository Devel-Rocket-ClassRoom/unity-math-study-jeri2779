...
public class CoordinateTransformDemo : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        childLocalPos = childObject.localPosition;              // 자식의 로컬 위치
        childWorldPos  = transform.TransformPoint(childLocalPos); // 로컬→월드 변환

        targetWorldPos = worldTarget.position;              // 타겟의 월드 위치    
        targetLocalPos = transform.InverseTransformPoint(targetWorldPos); // 월드→로컬 변환
        // TODO
