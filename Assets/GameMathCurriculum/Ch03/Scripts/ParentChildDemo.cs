...
public class ParentChildDemo : MonoBehaviour
{
    ...
    {
        // TODO
        if(autoRotate)// 자동 회전 활성화 시 부모 오브젝트를 회전
        {
            float deltaRotation = rotationSpeed * Time.deltaTime;// 이번 프레임에 회전할 각도
            transform.Rotate(0f, deltaRotation, 0f); // 부모 오브젝트를 Y축 기준으로 회전


        }
        unityWorldPos = childObject.position;  // Unity가 자동으로 계산한 자식의 월드 위치
        Vector3 childLocalPos = childObject.localPosition; // 자식의 로컬 위치

        // 부모의 TRS 행렬 생성
        Matrix4x4 parentMatrix = Matrix4x4.TRS(transform.position, transform.rotation, transform.localScale);

        manualWorldPos = parentMatrix.MultiplyPoint(childLocalPos); // 수동 계산한 자식의 월드 위치
        positionDifference = Vector3.Distance(unityWorldPos, manualWorldPos); // 두 위치 간의 차이 계산
