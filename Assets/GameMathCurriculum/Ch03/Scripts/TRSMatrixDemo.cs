...
public class TRSMatrixDemo : MonoBehaviour
{
    ...
    {
        // TODO
        Quaternion rotQuat = Quaternion.Euler(0f, rotationAngle, 0f);// 회전 쿼터니언 생성
        currentTRS = Matrix4x4.TRS(translation, rotQuat, scale);// TRS 행렬 생성 (S→R→T 순서)

        transformedPoint = currentTRS.MultiplyPoint(baseShape[1]); // 원점이 어떻게 이동하는지 계산
    ...
    private void OnDrawGizmos()
    {
        ...
        {
            // TODO
            var r = Matrix4x4.Rotate(rotQuat);          // 회전 행렬
            var t = Matrix4x4.Translate(translation);   // 이동 행렬
            var s = Matrix4x4.Scale(scale);             // 스케일 행렬
            Matrix4x4 rtsMatrix = r * t * s;            // r×t×s 순서로 곱한 행렬 (잘못된 순서)
            Color wrongColor = new Color(0.5f, 0.5f, 0.5f); // 빨강
            DrawShape(originPos, rtsMatrix,wrongColor, "잘못된 순서 (R×T×S)"); // 잘못된 순서 시각화

            Vector3 wrongOrigin = originPos + rtsMatrix.MultiplyPoint(Vector3.zero);
            DrawDashedLine(originPos, wrongOrigin, wrongColor); // 회전 단계 시각화
        }
