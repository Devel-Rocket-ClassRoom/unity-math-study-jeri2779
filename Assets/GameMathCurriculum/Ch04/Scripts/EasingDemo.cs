...
public class EasingDemo : MonoBehaviour
{
    ...
    {
        // TODO
        float jumpCurve = currentEasedValue * (1f - currentEasedValue) * 4f; // 단일 점프용 곡선 (0→1→0)
        ...
        if(useAnimationCurve)//커스텀 커브가 활성화된 경우 해당 커브에서 이징 값 계산
        {
        ...
        if(currentT >= 1f)//점프 애니메이션이 끝나면 원래 위치로 복귀
        {
            isJumping = false;
