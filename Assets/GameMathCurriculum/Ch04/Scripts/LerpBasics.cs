...
public class LerpBasics : MonoBehaviour
{
    ...
    private void UpdateHPLerp()
    {
        ...
        else
        {
            ...
            displayHP = Mathf.MoveTowards(displayHP, currentHP, hpLerpSpeed * Time.deltaTime);
    ...
    private void UpdatePositionTracking()
    {
        ...
        if (useAbsoluteLerp)
        {
            transform.position = Vector3.Lerp
                ...
                positionLerpSpeed * Time.deltaTime);//Lerp는 목표에 가까워질수록 속도가 느려지는 가속도감 있는 이동 방식
        }
        else
        {
            transform.position = Vector3.MoveTowards
                (transform.position, 
                targetTransform.position, 
                positionLerpSpeed * Time.deltaTime);//MoveTowards는 목표까지 일정한 속도로 이동하는 방식
        }
    }
    ...
    private void UpdateColorLerp()
    {
        ...
        // TODO
        colorLerpTime = (colorLerpTime + Time.deltaTime) % (2f / colorLerpSpeed); // 0~2초 사이를 반복

        float colorT = Mathf.PingPong(colorLerpTime * colorLerpSpeed, 1f); // 0~1 사이를 왕복하는 t값 계산

        materialInstance.color = Color.Lerp(colorStart, colorEnd, colorT); // 색상 보간 적용
        debugColorT = colorT; // 디버그용 t값 저장  
    }
