...
**스크립트:** `Assignment_DropTable.cs`

**목표:** 가중치 기반 누적 확률로 아이템을 선택하는 드롭 테이블 구현

**요구 사항:**
1. 드롭 테이블에 3가지 아이템이 가중치와 함께 정의되어 있다 (일반 70, 희귀 25, 전설 5)
2. 키 입력: Space(1회 드롭), R(10연차 드롭)
3. 드롭 시 모든 아이템의 가중치 합을 구하고, `[0, 총가중치)` 범위의 난수를 생성하여 누적 가중치 비교로 아이템을 결정한다
4. 드롭 결과와 확률 분포를 UI에 표시한다
5. 최근 20회 드롭 기록을 유지한다
         */



        totalWeight = 0f;
        foreach(var item in items)//누적 가중치 계산하여 드롭 결과 결정
        {
           totalWeight += item.weight;
        }
        if(totalWeight <= 0f)
        {
            Debug.LogWarning("  드롭이 불가능합니다.");
            return;
        }
