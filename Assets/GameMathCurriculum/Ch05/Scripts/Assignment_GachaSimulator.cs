...
public class Assignment_GachaSimulator : MonoBehaviour
{
    ...
    private void ExecutePull()
    {
        ...
        //if(currentPityCount < softPityStart)// 소프트 피티 구간에서는 확률 선형 증가
        //{
        //    currentEffectiveRate = baseRate;
        //}
        //else if(currentPityCount < hardPity)
        //{
        //    flaot rateIncrease = (1f - baseRate)  / (hardPity - softPityStart);
        //    currentEffectiveRate = baseRate + (currentPityCount - softPityStart + 1) * rateIncrease;
        //}
        //else// 하드 피티 도달 시 확률 100%로 SSR 확정
        //{
    ...
    private void UpdateUI()
    {
        ...
        if (ssrPityList.Count > 0)
        {
            ...
            for (int i = Mathf.Max(0,ssrPityList.Count - 3); i < ssrPityList.Count; i++)
            {
        ...
        expectedMsg += $"\n[예상값]\n";
        // float avgRate = (baseRate + currentEffectiveRate) / 2f; // ← 단순 평균 (부정확: 소프트 피티 구간 확률 증가 미반영)
        // 소프트 피티 이후: 현재 확률 ~ 100% 구간의 선형 평균
        // 소프트 피티 이전: 고정 구간(baseRate)과 선형 증가 구간의 가중 평균
        float avgRate = currentPityCount >= softPityStart
            ? (currentEffectiveRate + 1f) / 2f
            : (baseRate * (softPityStart - currentPityCount) +
               (baseRate + 1f) / 2f * (hardPity - softPityStart))
              / (hardPity - currentPityCount);
        if (avgRate > 0)
        {
