...
public class Assignment_PatrolGuard : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        // TODO 
        /*
         * 현재 forward → 목표 방향(dirToNext) 사이의 회전 차이를 구함
            → Quaternion.FromToRotation(transform.forward, dirToNext)
            → 이 회전 차이를 현재 회전에 합성하여 targetRotation 산출
         
         */
        Quaternion rotationToTarget = Quaternion.FromToRotation(transform.forward, dirToNext);
        targetRotation = rotationToTarget * transform.rotation;
        

        // TODO
        /*
         * 현재 회전에서 targetRotation으로 일정 각속도(turnSpeed)로 회전
            → Quaternion.RotateTowards(현재 회전, targetRotation, turnSpeed × deltaTime)
            → transform.rotation에 적용
         */
        float turnAngleSpeed = turnSpeed * Time.deltaTime;
        transform.rotation = Quaternion.RotateTowards(transform.rotation, targetRotation, turnAngleSpeed);
