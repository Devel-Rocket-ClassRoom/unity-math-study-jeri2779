...
public class LookRotationDemo : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        Vector3 toTarget = (target.position - transform.position).normalized;// 타겟 방향 계산    

        targetRotation = Quaternion.LookRotation(toTarget, Vector3.up);// 타겟 방향으로의 회전 계산

        angleDifference = Quaternion.Angle(transform.rotation, targetRotation);// 현재 회전과 타겟 회전 사이의 각도 차이 계산

        if (useSlerp)
        {
            //transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, slerpProgress);
            transform.rotation = Quaternion.Slerp(transform.rotation, targetRotation, turnSpeed * Time.deltaTime);

            //float angleToTarget = Quaternion.Angle(transform.rotation, targetRotation);
            //angleDifference = angleToTarget;
            //float step = turnSpeed * Time.deltaTime;
            //slerpProgress = Mathf.Clamp01(step / angleToTarget);
            slerpProgress = Mathf.Clamp01(angleDifference / 180f);
        }
        else
        {
