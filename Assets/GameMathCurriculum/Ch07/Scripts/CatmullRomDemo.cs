...
public class CatmullRomDemo : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        else
        {
            ...
            //float t = (elapsedTime / 10f) * speed; // 10초에 한 바퀴 도는 속도
            //currentT = loopPath ? t % 1f : Mathf.Clamp01(t);
    ...
    private Vector3 CatmullRom(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, float t)
    {
        ...
        Vector3 v1 = 2f * p1;
        Vector3 v2 = (-p0 + p2) * t;
        Vector3 v3 = (2f * p0 - 5f * p1 + 4f * p2 - p3) * t2;
        Vector3 v4 = (-p0 + 3f * p1 - 3f * p2 + p3) * t3;
        var res = 0.5f * (v1 + v2 + v3 + v4);
    ...
    private Vector3 EvaluateSpline(float t)
    {
        ...
        int numSegments = loopPath ? n : n - 1;
        float globalT = t * numSegments;
    ...
    private Vector3 GetWaypoint(int index)
    {
        // TODO
        int n = waypoints.Length;
        int wrappedIndex = loopPath ? (index + n) % n : Mathf.Clamp(index, 0, n - 1);
