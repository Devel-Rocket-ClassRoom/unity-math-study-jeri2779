...
public class CoordinatePipelineDemo : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        screenCoord  = Input.mousePosition;                             // 마우스 위치를 스크린 좌표로 가져오기
        viewportCoord = cam.ScreenToViewportPoint(screenCoord);         // 뷰포트 좌표로 변환
        ndcCoord = new Vector2(
            viewportCoord.x * 2f - 1f, 
            viewportCoord.y * 2f - 1f); // NDC 좌표로 변환
        Vector3 screenPointWithDepth = screenCoord;
        screenPointWithDepth.z = worldDepth; // ScreenToWorldPoint에 사용할 z값 설정
        worldCoord = cam.ScreenToWorldPoint(screenPointWithDepth); // 월드 좌표로 변환
