...
public class ScreenToWorldDemo : MonoBehaviour
{
    ...
    {
        // TODO
        currentScreenPos = Input.mousePosition; // 마우스 위치를 스크린 좌표로 가져오기
        var currentScreenPosWithDepth = currentScreenPos;
        currentScreenPosWithDepth.z = distanceFromCamera; // ScreenToWorldPoint에 사용할 z값 설정

        currentWorldPos = cam.ScreenToWorldPoint(currentScreenPosWithDepth); // 월드 좌표로 변환

        cursorFollower.position = currentWorldPos; // 커서 추적 오브젝트 위치 업데이트
